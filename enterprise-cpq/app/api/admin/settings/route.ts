...
export async function PUT(request: NextRequest) {
  try {
    ...
    const { data: user } = await supabaseAdmin
      ...
      .eq(isEnvAdmin ? 'username' : 'id', isEnvAdmin ? session.username! : session.userId!)
      .single()

    if (!user) return NextResponse.json({ error: 'User not found' }, { status: 404 })
    const dbUserId = (user as { id: string }).id
    ...
    const { error } = await supabaseAdmin.from('users').update(updates).eq('id', dbUserId)
    ...
    updatedSession.userId = dbUserId
