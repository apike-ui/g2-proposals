...
-- ============================================================
-- Enterprise Multiproduct Build — G2 Proposal Builder Tables
-- ============================================================

-- G2 SKU availability overrides (admin controls which SKUs are visible to users)
CREATE TABLE IF NOT EXISTS g2_sku_overrides (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  sku_id TEXT NOT NULL UNIQUE,          -- matches id in FOUNDATION_TIERS / ADDON_CATALOG / NONAVC_CATALOG
  sku_type TEXT NOT NULL CHECK (sku_type IN ('foundation', 'addon', 'nonavc')),
  enabled BOOLEAN DEFAULT true,
  custom_data JSONB DEFAULT '{}',       -- per-tier price overrides at catalog level
  updated_at TIMESTAMPTZ DEFAULT NOW()
);

ALTER TABLE g2_sku_overrides ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Service role full access on g2_sku_overrides" ON g2_sku_overrides FOR ALL TO service_role USING (true) WITH CHECK (true);

-- Rate cards (per-customer negotiated pricing)
CREATE TABLE IF NOT EXISTS rate_cards (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  name TEXT DEFAULT 'Untitled Rate Card',
...
CREATE POLICY "Service role full access on rate_cards" ON rate_cards FOR ALL TO service_role USING (true) WITH CHECK (true);
...
-- G2 Proposals
CREATE TABLE IF NOT EXISTS g2_proposals (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
...
ALTER TABLE g2_proposals ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Service role full access on g2_proposals" ON g2_proposals FOR ALL TO service_role USING (true) WITH CHECK (true);

DROP TRIGGER IF EXISTS update_g2_proposals_updated_at ON g2_proposals;
CREATE TRIGGER update_g2_proposals_updated_at
  BEFORE UPDATE ON g2_proposals
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- Proposal versions (full JSON snapshot per save)
CREATE TABLE IF NOT EXISTS proposal_versions (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  proposal_id UUID REFERENCES g2_proposals(id) ON DELETE CASCADE NOT NULL,
...
CREATE POLICY "Service role full access on proposal_versions" ON proposal_versions FOR ALL TO service_role USING (true) WITH CHECK (true);

-- Add rate_card_id to quotes (allows CPQ quotes to reference a G2 rate card)
ALTER TABLE quotes ADD COLUMN IF NOT EXISTS rate_card_id UUID REFERENCES rate_cards(id) ON DELETE SET NULL;
